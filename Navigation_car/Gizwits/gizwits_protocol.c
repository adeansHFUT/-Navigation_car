...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSTART)
    {
        info->event[info->num] = EVENT_START;
        info->num++;
        dataPoints->valueSTART = gizStandardDecompressionValue(START_BYTEOFFSET,START_BITOFFSET,START_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagEND)
    {
        info->event[info->num] = EVENT_END;
        info->num++;
        dataPoints->valueEND = gizStandardDecompressionValue(END_BYTEOFFSET,END_BITOFFSET,END_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSTART != cur->valueSTART)
    {
        GIZWITS_LOG("valueSTART Changed\n");
        ret = 1;
    }
    if(last->valueEND != cur->valueEND)
    {
        GIZWITS_LOG("valueEND Changed\n");
        ret = 1;
    }
    if(last->valueObstacle != cur->valueObstacle)
    {
        GIZWITS_LOG("valueObstacle Changed\n");
        ret = 1;
    }

    if(last->valueCurrentPoint != cur->valueCurrentPoint)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCurrentPoint Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    if(dataPoints->valueSTART >= START_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueSTART Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(START_BYTEOFFSET,START_BITOFFSET,START_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSTART);  
    }
    if(dataPoints->valueEND >= END_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueEND Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(END_BYTEOFFSET,END_BITOFFSET,END_LEN,(uint8_t *)devStatusPtr,dataPoints->valueEND);  
    }
    gizStandardCompressValue(Obstacle_BYTEOFFSET,Obstacle_BITOFFSET,Obstacle_LEN,(uint8_t *)devStatusPtr,dataPoints->valueObstacle);
    ...
    devStatusPtr->valueCurrentPoint = gizY2X(CurrentPoint_RATIO,  CurrentPoint_ADDITION, dataPoints->valueCurrentPoint); 
